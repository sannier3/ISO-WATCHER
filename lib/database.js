...
/** better-sqlite3 n'accepte pas les booléens JS — 0/1 uniquement. */
...
/** Filtre temporel « depuis X heures » (MySQL vs SQLite). */
export function sqlDetectedSince(column, hours, driver) {
  const h = Math.max(1, Number(hours) || 24);

  if (driver === 'mysql') {
    return {
      clause: `${column} >= DATE_SUB(NOW(), INTERVAL ? HOUR)`,
      params: [h]
    };
  }
...
async function initMysql(pool, logger) {
  const statements = [
    ...
    `CREATE TABLE IF NOT EXISTS destinations (
      ...
      destination_type ENUM(
        'email', 'discord_webhook', 'teams_webhook', 'generic_webhook',
        'slack_webhook'
      ) NOT NULL,
  ...
  const migrations = [
    ...
    'DELETE FROM destinations WHERE destination_type = \'matrix\'',
    `ALTER TABLE destinations MODIFY COLUMN destination_type ENUM(
      'email', 'discord_webhook', 'teams_webhook', 'generic_webhook',
      'slack_webhook', 'telegram', 'ntfy', 'pushover'
    ) NOT NULL`,
    'DELETE FROM destinations WHERE destination_type IN (\'telegram\', \'ntfy\', \'pushover\')',
    `ALTER TABLE destinations MODIFY COLUMN destination_type ENUM(
      'email', 'discord_webhook', 'teams_webhook', 'generic_webhook',
      'slack_webhook'
    ) NOT NULL`

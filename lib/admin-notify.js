/**
 * Notifications administrateur : e-mail, Discord, Teams, webhook générique, stockage UI.
 */
...
const VALID_CHANNELS = new Set(['email', 'discord', 'teams', 'webhook', 'ui']);

function parseChannelList(raw, fallback = ['email', 'ui']) {
...
async function postJson(targetUrl, payload, { timeoutMs = 30000, extraHeaders = {} } = {}) {
  return new Promise((resolve, reject) => {
    ...
    req.on('timeout', () => req.destroy(new Error(`Timeout webhook sur ${targetUrl}`)));
    req.on('error', reject);
    req.write(body);
    req.end();
  });
...
export function createAdminNotify({ config, mailer, log, dataDir }) {
  ...
  const defaultChannels = parseChannelList(process.env.ADMIN_NOTIFY_CHANNELS, ['email', 'ui']);
  const instantChannels = parseChannelList(
    process.env.ADMIN_INSTANT_NOTIFY_CHANNELS || process.env.ADMIN_NOTIFY_CHANNELS,
    defaultChannels
  );

  const adminCfg = {
    email: String(config.admin?.email || process.env.ADMIN_EMAIL || '').trim(),
    ...
    instantNotifyChannels: config.admin?.instantNotifyChannels?.length
      ...
      : instantChannels,
    discordWebhook: String(config.admin?.discordWebhook || '').trim(),
    teamsWebhook: String(config.admin?.teamsWebhook || '').trim(),
    genericWebhook: String(config.admin?.genericWebhook || '').trim(),
    smtp: config.smtp,
  ...
  function getPublicConfig() {
    return {
      channels: {
        email: Boolean(adminCfg.email),
        discord: Boolean(adminCfg.discordWebhook),
        teams: Boolean(adminCfg.teamsWebhook),
        webhook: Boolean(adminCfg.genericWebhook),
        ui: true
      },
  ...
  async function sendDiscordEmbeds(embeds) {
    ...
    for (const chunk of chunks) {
      await postWebhook(adminCfg.discordWebhook, { embeds: chunk }, log, 'Discord');
    }
  }
  ...
  async function sendTeamsAdaptiveCard(title, text, facts = []) {
    ...
    await postWebhook(adminCfg.teamsWebhook, payload, log, 'Teams');
  }
  ...
  async function sendGenericWebhook(event, data) {
    ...
    await postWebhook(adminCfg.genericWebhook, { event, ...data }, log, 'Webhook');
  }

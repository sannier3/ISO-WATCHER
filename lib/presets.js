...
export function createPresetsService({ rootDir, config, pool }) {
  ...
  async function findDbSourceForPresetKey(isoItemId, presetId, sourceKey, binding) {
  ...
  async function computePresetDrift(preset, state) {
    ...
    if (!isoRow) {
      return {
        ...
        iso_changes: [],
        source_changes: [],
        missing_sources: preset.sources.map((s) => s.key),
    ...
    for (const source of preset.sources) {
      const dbSource = await findDbSourceForPresetKey(isoRow.id, preset.id, source.key, binding);
    ...
    const update_available = iso_changes.length > 0 || source_changes.length > 0 || missing_sources.length > 0;
    const parts = [];
    ...
    if (missing_sources.length) {
      parts.push(`${missing_sources.length} source(s) à ajouter`);
    }
    ...
    return {
      ...
      summary: update_available
        ? `Mise à jour disponible : ${parts.join(', ')}`
        : 'À jour avec le catalogue'
    };
  }
  ...
  async function getDriftForIsoItem(isoRow) {
    ...
    const preset = catalog.presets.find((p) => p.id === isoRow.catalog_preset_id);
  ...
  async function listPresets({ tag, q } = {}) {
    ...
    for (const preset of filtered) {
      ...
      rows.push({
        ...
        source_count: preset.sources.length,
        imported: drift.status !== 'not_imported',
        import_status: drift.status,
        linked_iso_item_id: drift.iso_item_id,
        update_available: drift.update_available,
        drift_summary: drift.summary,
        drift_counts: {
          iso: drift.iso_changes.length,
          sources: drift.source_changes.length,
          missing_sources: drift.missing_sources.length
        },
        catalog_kind: getActiveCatalogKind()
      });
    }

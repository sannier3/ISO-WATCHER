...
export function loadConfig() {
  ...
  return {
    ...
    admin: {
      ...
      notifyChannels: String(process.env.ADMIN_NOTIFY_CHANNELS || 'email,ui')
        .split(',')
        .map((s) => s.trim().toLowerCase())
        .filter(Boolean),
      instantNotifyChannels: String(
        process.env.ADMIN_INSTANT_NOTIFY_CHANNELS || process.env.ADMIN_NOTIFY_CHANNELS || 'email,ui'
      )
        .split(',')
        .map((s) => s.trim().toLowerCase())
        .filter(Boolean),
      discordWebhook: String(process.env.ADMIN_DISCORD_WEBHOOK_URL || '').trim(),
      teamsWebhook: String(process.env.ADMIN_TEAMS_WEBHOOK_URL || '').trim(),
      genericWebhook: String(process.env.ADMIN_WEBHOOK_URL || '').trim(),
      slackWebhook: String(process.env.ADMIN_SLACK_WEBHOOK_URL || '').trim(),
      telegramBotToken: String(process.env.ADMIN_TELEGRAM_BOT_TOKEN || '').trim(),
      telegramChatId: String(process.env.ADMIN_TELEGRAM_CHAT_ID || '').trim(),
      ntfyServer: String(process.env.ADMIN_NTFY_SERVER || 'https://ntfy.sh').trim(),
      ntfyTopic: String(process.env.ADMIN_NTFY_TOPIC || '').trim(),
      ntfyPriority: String(process.env.ADMIN_NTFY_PRIORITY || 'default').trim(),
      ntfyTags: String(process.env.ADMIN_NTFY_TAGS || 'iso-watcher').trim(),
      pushoverUserKey: String(process.env.ADMIN_PUSHOVER_USER_KEY || '').trim(),
      pushoverAppToken: String(process.env.ADMIN_PUSHOVER_APP_TOKEN || '').trim(),
      matrixHomeserver: String(process.env.ADMIN_MATRIX_HOMESERVER || '').trim(),
      matrixAccessToken: String(process.env.ADMIN_MATRIX_ACCESS_TOKEN || '').trim(),
      matrixRoomId: String(process.env.ADMIN_MATRIX_ROOM_ID || '').trim()
    },
    linkCheck: {
    ...
    presets: {
      remoteUrl: String(
        process.env.PRESETS_CATALOG_URL
        || 'https://raw.githubusercontent.com/sannier3/ISO-WATCHER/main/presets/catalog.json'
      ).trim(),
      preferRemote: parseBool(process.env.PRESETS_PREFER_REMOTE, false)
    },
    security: {
